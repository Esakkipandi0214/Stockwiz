...
const Pricing: React.FC = () => {
  ...
  return (
    ...
    <div className="container px-4 mx-auto">
      ...
      <div className="flex flex-wrap items-stretch gap-4 md:gap-12 justify-center">
        {(year ? Yearplans : LifeTimeplans).map((plan, index) => (
          ...
          >
            <div
              className={`flex flex-grow flex-col p-4 sm:p-6 space-y-4 sm:space-y-6 rounded-2xl sm:rounded-3xl shadow-lg text-white transition-all duration-300 ease-in-out transform ${plan.cardStyle}`}
            >
              <div className="space-y-1">
                <h4 className="text-xl sm:text-2xl md:text-3xl text-[#d100d1] font-medium my-2">{plan.title}</h4>
                <span className="text-2xl sm:text-3xl md:text-4xl font-bold">
                  {plan.price}
                  {plan.priceDescription && (
                    <span className="text-xs sm:text-base ml-2 font-normal tracking-wide text-white/70">{plan.priceDescription}</span>
                  )}
                </span>
                <p className="text-xs sm:text-sm text-white/80">{plan.description}</p>
              </div>
                ...
                <ul className="space-y-2">
                  {plan.features.map((feature, featureIndex) => (
                    <li key={featureIndex} className="flex items-center space-x-2">
                      ...
                      <span className="text-xs sm:text-sm text-white/80">{feature.feature}</span>
                    </li>
                  ))}
                </ul>
              </div>
